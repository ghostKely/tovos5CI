...
class C_Renew extends CI_Controller {
...
/* FONCTION POUR AVOIR LA LISTE DES CANDIDATS QUI FONT UN ENTRETIEN */
    public function getListeRenew() {
        $data['pageTitle'] = "Liste Candidat Renew";                       //titre de la page de destination
        $data['pageToLoad'] = "renew/Renew_List";                   //path de a page de destination

        $sql = "SELECT * FROM v_candidatentretien WHERE idrenouvellement IS NOT NULL";
        $data['listeCandidatRenew'] = $this->dao->executeQuery($sql); //liste des besoins non valide
...
/* FONCTION POUR RECALLER UN CANDIDAT A ENTRETIEN */
    ...
    public function embaucheCandidat($valeur = null) {
        ...
        $lastBesoin = $this->insertBesoinValide($idCandidat, $idBesoin);
    ...
    public function insertBesoinValide($idCandidat, $idBesoin) {
        $sql = "SELECT *
                    FROM BesoinValide
                    WHERE idBesoin = '".$idBesoin."'
                    ORDER BY idBesoinValide DESC
                    LIMIT 1;";
        $lastBesoin = $this->dao->executeQuery($sql);
        var_dump($sql, $lastBesoin);
        $to_insert = [
            'datevalidationrh' => $lastBesoin[0]['datevalidationrh'],
            'checkrh' => $lastBesoin[0]['checkrh'],
            'averagenoteqcm' => $lastBesoin[0]['averagenoteqcm'],
            'datevalidationdg' => $lastBesoin[0]['datevalidationdg'],
            'checkdg' => $lastBesoin[0]['checkdg'],
            'statutpostulation' => true,
            'idbesoin' => $idBesoin
        ];

        $checkInsert = $this->dao->insert_into('besoinvalide', $to_insert);
            if ($checkInsert !== false) {                                    //si la valeur de $inserted n'est pas null alors insertion réussi
                $this->session->set_flashdata('checkValidation', [          //redirect vers page d'insertion + popup de validation de insertion réussi
                    'message' => "Annonce supprime !"        //message de validation pour page Validation.php
                    ]);
            } else {                                                    //si la valeur de $inserted est pas null alors insertion raté                
                $this->session->set_flashdata('checkValidation', [          //redirect vers page d'insertion + popup de validation de insertion raté
                    'message' => "Echec de la suppression de annonce !"                 //message de validation pour page Validation.php
                ]);
            }
        return true;
    }
